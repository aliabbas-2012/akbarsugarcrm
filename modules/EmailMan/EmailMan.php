...
// if
    ...
    function sendEmail($mail, $save_emails = 1, $testmode = false) {
        ...
        $assinged_user = '';
        if (!empty($module->assigned_user_id)) {
            require_once('include/OutboundEmail/OutboundEmail.php');
            require_once($beanFiles["User"]);
            $assinged_user = new User();

            $assinged_user = $assinged_user->retrieve($module->assigned_user_id);
            $mail->setCustomMailer($assinged_user);
        }
        ...
        if ((!isset($module->email_opt_out) || ($module->email_opt_out !== 'on' && $module->email_opt_out !== 1 && $module->email_opt_out !== '1')) && (!isset($module->invalid_email) || ($module->invalid_email !== 'on' && $module->invalid_email !== 1 && $module->invalid_email !== '1'))) {
            ...
            if (!empty($assinged_user)) {
                $mail->Sender = $assinged_user->email1;
                $mail->From = $assinged_user->email1;
                $mail->FromName = $assinged_user->name;
            }
